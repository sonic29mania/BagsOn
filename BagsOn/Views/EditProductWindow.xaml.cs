...
namespace BagsOn.Views
{
    public partial class EditProductWindow : Window
    {
        ...
        private ProductEditData? _editData;

        private readonly List<string> _selectedImagePaths;

        private bool _replaceImages;

        public EditProductWindow(int productId, int variantId)
        {
            InitializeComponent();
        ...
        {
            _editData = await _productRepository.GetProductForEditAsync(_productId, _variantId);

            if (_editData == null)
            {
                MessageBox.Show("Не вдалося завантажити товар для редагування.");
                DialogResult = false;
                Close();
                return;
            }

            ModelNameTextBox.Text = _editData.ModelName;

            BrandComboBox.SelectedValue = _editData.BrandId;
            TypeComboBox.SelectedValue = _editData.TypeId;
            MaterialComboBox.SelectedValue = _editData.MaterialId;
            ColorComboBox.SelectedValue = _editData.ColorId;

            PriceTextBox.Text = _editData.Price.ToString(CultureInfo.InvariantCulture);
            QuantityTextBox.Text = _editData.Quantity.ToString();
            ArrivalDatePicker.SelectedDate = _editData.ArrivalDate;

            foreach (object item in CategoryListBox.Items)
            {
                if (item is ReferenceItem category &&
                    _editData.CategoryIds.Contains(category.Id))
                {
                    CategoryListBox.SelectedItems.Add(item);
                }
            }

            _selectedImagePaths.Clear();

            foreach (string imagePath in _editData.ImagePaths)
            {
        ...
        private void ChooseImagesButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (dialog.ShowDialog() == true)
            {
                ...
                ImagesInfoTextBox.Text = $"Обрано нових фото: {_selectedImagePaths.Count}";
        ...
        private async void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                await _productRepository.UpdateProductVariantAsync(
                    _productId,
                    _variantId,
            ...
            {
                MessageBox.Show("Помилка при редагуванні товару:\n" + ex.Message);
            }
        }
        ...
        {
            ImagesInfoTextBox.Text = $"Фото: {_selectedImagePaths.Count}";
